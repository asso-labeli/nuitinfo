...
mongoose.model('User')
    ...
    .select('firstName lastName email school team')
    .populate('school.institution')
    .exec((err, users) => {
        if (err) {
            return console.error(err);
        }

        console.log('Ecole;Année d\'étude;Prénom;Nom;Parcours');

        for (let user of users) {
            if (!user.team ||
                user.school.institution.name !== 'Université de Bordeaux' ||
                user.school.pathway.toLowerCase() === 'iut info' ||
                user.school.pathway.toLowerCase() === 'dawin' ||
                user.school.pathway.toLowerCase().indexOf('adsillh') > -1 ||
                user.school.pathway.toLowerCase().indexOf('cognitique') > -1) {
                continue;
            }
